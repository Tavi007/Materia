            ...
            for (int dvRange = 0; dvRange <= rangeLevel * 2; dvRange++) {
                for (int dvHeight = -heightLevel; dvHeight <= heightLevel; dvHeight++) {
                    for (int dvWidth = -widthLevel; dvWidth <= widthLevel; dvWidth++) {
                        ...
                        int dx = (int) Math.round(vRange.x() * dvRange + vHeight.x() * dvHeight + vWidth.x() * dvWidth);
                        int dy = (int) Math.round(vRange.y() * dvRange + vHeight.y() * dvHeight + vWidth.y() * dvWidth);
                        int dz = (int) Math.round(vRange.z() * dvRange + vHeight.z() * dvHeight + vWidth.z() * dvWidth);
