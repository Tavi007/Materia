...
import Tavi007.Materia.network.SSpawnApOrbPacket;
...
public class ApOrbEntity extends Entity {
	   public int apColor;
	   public int apOrbAge;
	   ...
	   private int apTargetColor;

	   public ApOrbEntity(World worldIn, double x, double y, double z, int expValue) {
	      ...
	      this.apValue = expValue;
	   }

	   public ApOrbEntity(EntityType<? extends ExperienceOrbEntity> p_i50382_1_, World entity) {
	   ...
	   public void tick() {
	      ...
	      double d0 = 8.0D;
	      if (this.apTargetColor < this.apColor - 20 + this.getEntityId() % 100) {
	         ...
	         this.apTargetColor = this.apColor;
	      }
	   ...
	   public boolean attackEntityFrom(DamageSource source, float amount) {
	      ...
	      if (this.isInvulnerableTo(source)) {
	         ...
	         this.apOrbHealth = (int)((float)this.apOrbHealth - amount);
	   ...
	   public void writeAdditional(CompoundNBT compound) {
	      compound.putShort("Health", (short)this.apOrbHealth);
	      compound.putShort("Age", (short)this.apOrbAge);
	      compound.putShort("Value", (short)this.apValue);
	   }
	   ...
	   public void readAdditional(CompoundNBT compound) {
	      this.apOrbHealth = compound.getShort("Health");
	      this.apOrbAge = compound.getShort("Age");
	      this.apValue = compound.getShort("Value");
	   }
	         ...
	         if (this.delayBeforeCanPickup == 0 && entityIn.xpCooldown == 0) {
	            ...
	            if (entry != null) {
	               ...
	               if (!itemstack.isEmpty() && itemstack.isDamaged()) {
	                  int i = Math.min((int)(this.apValue * itemstack.getXpRepairRatio()), itemstack.getDamage());
	                  this.apValue -= this.durabilityToAp(i);
	                  itemstack.setDamage(itemstack.getDamage() - i);
	               }
	            }

	            if (this.apValue > 0) {
	               entityIn.giveExperiencePoints(this.apValue);
	            }
	   ...
	   private int durabilityToAp(int durability) {
	      return durability / 2;
	   }

	   private int apToDurability(int xp) {
	      return xp * 2;
	   }
	    ...
	    */
	   @OnlyIn(Dist.CLIENT)
	   public int getTextureByAP() {
	      if (this.apValue >= 2477) {
	         return 10;
	      } else if (this.apValue >= 1237) {
	         return 9;
	      } else if (this.apValue >= 617) {
	         return 8;
	      } else if (this.apValue >= 307) {
	         return 7;
	      } else if (this.apValue >= 149) {
	         return 6;
	      } else if (this.apValue >= 73) {
	         return 5;
	      } else if (this.apValue >= 37) {
	         return 4;
	      } else if (this.apValue >= 17) {
	         return 3;
	      } else if (this.apValue >= 7) {
	         return 2;
	      } else {
	         return this.apValue >= 3 ? 1 : 0;
	      }
	   }
	    ...
	    */
	   public static int getAPSplit(int expValue) {
	      if (expValue >= 2477) {
	         return 2477;
	   ...
	   public IPacket<?> createSpawnPacket() {
	      return new SSpawnApOrbPacket(this);
	   }
	}
